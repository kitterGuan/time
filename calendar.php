...
class Calendar{ 
	...
	*/ 
	...
	protected function _showDay($i,$y){
	...
	*/ 
	protected function _showDate() { 
		...
		for ($i=1;$i<=$this->_days;$i++){//输出天数信息 
			$td = $this->_year."-".$this->_month."-".$i;
			$aa = intval(intval(strtotime($td) - intval(strtotime($this->_begin)))/86400);
			if ($nums%7==0){//换行处理：7个一行 
				if($this->tm == 1 && $i == date("d")){
					$this->_table.="<td style='color:red'>$i</td></tr><tr>"; 
				}else{
					if($aa%3 != 0){
						$this->_table.="<td style='color:blue'>$i</td></tr><tr>";
					}else{
				...
				if($this->tm == 1 && $i == date("d")){
					$this->_table.="<td style='color:red'>$i</td>"; 
				}else{
					if($aa%3 != 0){
						$this->_table.="<td style='color:blue'>$i</td>"; 
					}else{						
					$this->_table.="<td>$i</td>"; 
					}
				}
